...
```
**검색·생성 호출** (모두 `AiWebService` → `PrecedentRagClient`(HTTP) → Python)
```
[브라우저] /api/ai/* → AiController(caseId 시 소유권 확인) → AiWebService
  요약(/summary)            : Python /llm/summarize (변호사 전용)
  키워드(/keywords)         : Python /llm/extract-keywords  ※추출=미리보기(저장 안 함)
  키워드 저장(/keywords/save): 사건 keyword + 변호사 specialty + AI이력 저장
  유사판례(/similar..)      : Python /search → case_id+score → JPA PrecedentRepository로 본문 조립
  법리(/legal-rules)        : Python /rag/analyze-rules (판례 검색+LLM, JSON 실패 시 산문 폴백)
```
**요청–호출–저장(영속화) — 단일 JPA 경로**
```
AiPersistenceService.save (Spring Data JPA)
  ① ai_analysis_request 저장  ② 기능 실행  ③ ai_analysis_result 저장(case_id 연결)
→ 요약·키워드·법리·유사판례 모두 항상 저장, 저장 실패는 로그로 노출
→ 사건 상세 "AI 결과 이력"에 누적 표시
```
> 역할 제한: 요약·유사판례 = 변호사 전용, 키워드·법리 = 의뢰인 포함 전체.
> caseId 지정 시 **사건 소유권 확인**(남의 사건에 쓰기 차단).

### 4.5 변호사 검색 (키워드 — 벡터 의미 유사도)
```
/lawyers?keyword=&specialty=&region=  또는  caseId(저장된 사건 키워드)
 → LawyerService.search
    · 지역/분야 : 하드 필터
    · 키워드    : 후보 변호사 프로필(전문분야+소개)을 Python /embed/rerank 로 보내
                 BGE-M3 코사인 유사도로 정렬 ("정보보호"↔"개인정보"처럼 글자 안 겹쳐도 매칭)
    · 폴백      : RAG 장애 시 부분문자열 매칭 + 업무량 정렬
사건 키워드 흐름: AI 키워드 추출 → 사건에 저장 → 변호사 찾기에서 그 키워드로 의미 검색
```
...
사무직원: 상담 완료(APPROVED → COMPLETED)   ★ 수임의 전제조건
의뢰인: 거절/취소 건 삭제
```
> **상담이 COMPLETED 되어야** 그 변호사에게 수임을 요청할 수 있다(§4.7).

### 4.7 수임 상태머신
> **전제조건**: 해당 변호사와의 상담이 `COMPLETED` 여야 의뢰인이 수임을 요청할 수 있다.
> (RetainerService가 검증 / 수임 화면은 상담 완료 변호사만 드롭다운에 노출)
```
...
### 4.9 사건 배당 (대표변호사)
```
대표변호사: /assign → 수임(RETAINED) 사건 목록
 → 변호사 선택(업무량 적은 순) → 배당
 → assigned_lawyer 변경 + 업무량(currentWorkload) 자동 조정(이전 -1, 신규 +1)
배당된 사건은 그 변호사의 /my-cases(담당 사건)에 즉시 표시.
```

### 4.10 사건 소유권 (데이터 접근제어)
```
역할별 접근 규칙(CaseService.canAccess):
  의뢰인 = 본인 사건  /  변호사 = 배당된 사건  /  사무직원 = 전체(행정)
적용: 사건 상세·증거·AI(caseId 지정)·사건 API·변호사검색 사건선택
위반 → AccessDeniedException → GlobalExceptionHandler
  페이지 → /access-denied,  /api/** → 403
```
> 화면 메뉴 숨김(UX) + 역할(@RoleAllowed) + 데이터 소유권의 **3중 방어**.
> 단, 배당 화면(/assign)은 대표의 행정 기능이라 사무소 전체 RETAINED 사건을 본다.
...
web/                         ← 웹 계층(Spring Boot)
 ...
 │              RetainerController, AssignmentController, PlaceholderPageController
 ├ service/     AuthService, AiWebService, AiPersistenceService, CaseService,
 │              LawyerService, ConsultationService, RetainerService, AssignmentService
 ...
 ├ auth/        SessionConst, LoginMember, RoleAllowed, LoginInterceptor, AccessDeniedException
 ├ config/      WebConfig(인터셉터 등록), GlobalExceptionHandler(소유권 위반→403/redirect)
...
ai/                          ← AI 도메인(enum + 레거시 기능 객체)
 AnalysisType(웹에서 사용), AIAnalysisFunction, AIRequestStatus, AIAnalysisService,
 CaseSummary, CaseKeywordsExtract, LegalRuleAnalysis, SimilarPrecedentsAnalysis ...
 ※ AIAnalysisService/SimilarPrecedentsAnalysis 등 JDBC 경로는 **웹 미사용**(콘솔/레거시).
   웹 AI 영속화는 AiWebService → AiPersistenceService(JPA).

precedent/                   ← 판례 + RAG 연동
 PrecedentRagClient(HTTP/1.1: 검색/요약/키워드/법리/rerank), PrecedentHit,
 SummaryDto, LegalRulesDto, Precedent(도메인), PrecedentDAO(JDBC, ※웹 미사용·레거시),
 PrecedentImporter, RagConnectionTest, Step2Test

legalCase/, consultation/, retainer/   ← 도메인 enum/클래스 (CaseCategory/Status 등)
db/ , db.dao/                ← JDBC(DBA, DAO), DBInitializer  ※웹 실행 경로 미사용(콘솔/레거시 전용)
...
> 웹 실행 경로의 **DB 접근은 100% Spring Data JPA**. (JDBC DAO·`AIAnalysisService`·`PrecedentDAO`는 콘솔/레거시 전용으로만 잔존)
...
rag/
 ...
 ├ retriever.py   dense(BGE-M3)+BM25+RRF, case_id 그룹핑, rerank(임의 문서 코사인)
 ...
 └ api.py         FastAPI: /healthz /search/similar-precedents /answer
                  ...
                  /embed/rerank (BGE-M3 코사인 — 변호사 의미 검색)
...
| `POST /embed/rerank` | query, documents[{id,text}] | {results[{id, score}]} — 코사인 유사도 내림차순 |
...
- **ConsultationStatus**: REQUESTED / APPROVED / **COMPLETED**(수임 전제) / REJECTED / SCHEDULE_CHANGED / CANCELED
...
| **사건 소유권 접근제어** | 역할별 데이터 격리(의뢰인/변호사/직원) — AccessDeniedException |
| **변호사 벡터 검색** | 키워드↔프로필 의미 유사도(BGE-M3 rerank) — 부분문자열 한계 극복 |
| **AI 영속화 JPA 일원화** | 요약 포함 모든 AI 저장을 AiPersistenceService(JPA)로 → 웹 경로 100% JPA |
| **상담 완료 → 수임** | 유스케이스 전제조건을 서비스+화면에서 강제 |
...
**완료**: 인증·역할권한·**사건 소유권 접근제어**·디자인(Cohere)·사건(등록/상세/증거/AI이력)·**변호사 벡터(의미) 검색**·AI 4종(요약/키워드/유사판례/법리)+**JPA 영속화 일원화**·상담(신청/일정/승인/**완료**/삭제)·수임(**상담완료 전제**·요청/조건/수락/거절/조정요청·재제안/확정/삭제)·**사건 배당**(업무량 조정)·진행상황 공유·열람·담당 사건·판례 기반 시드.

**미구현/단순화**: 배당 변호사 추천 AI(RecommendLawyers), 서면 초안(DocumentDraft), 본인인증/전자서명.

**기술 정리**: 웹 실행 경로는 **100% Spring Data JPA**로 일원화됨(레거시 JDBC DAO·`AIAnalysisService`·`PrecedentDAO`는 콘솔/독립 실행용으로만 잔존). 전역 예외처리는 소유권 위반 한정(@ControllerAdvice) — 입력검증(@Valid) 보강 여지.

---
*문서 기준: 사건 소유권 접근제어 · 사건 배당 · 변호사 벡터검색 · AI/DB JPA 일원화 · 상담완료→수임 반영 시점.*
